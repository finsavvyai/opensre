...
If you want a local stack, use the official SigNoz Docker setup (or the thin wrappers in `demo/signoz/` that mirror it):

```bash
# optional convenience wrappers (mirror official docs)
bash demo/signoz/start-local.sh
# ... later
bash demo/signoz/stop-local.sh
```
...
- **`query_signoz_metrics`** — Query `signoz_metrics.distributed_samples_v4` for CPU, memory, error rate, latency percentiles.
...
SigNoz emits standard Prometheus Alertmanager webhooks. To trigger OpenSRE investigations automatically:
...
quantiles directly from span durations.
